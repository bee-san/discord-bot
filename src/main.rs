...
async fn ciphey(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    let ciphey_api_url = format!("https://pl8u5p7v00.execute-api.us-east-2.amazonaws.com/default/ciphey_lambda_api?ctext={}", message);
    
    // Create a new reqwest client
    let client = reqwest::Client::new();

    // Make a GET request to the specified URL
    let response = client.get(ciphey_api_url)
        .send()
        .await?;
    ...
    let my_string = "{\"message\": \"Internal server error\"}";
        ...
        .send_message(&ctx.http, |m| {
            m.content(&tag_user).embed(|e| {
                e.title("😭 Error: Your text could not be decoded")
                    .field("You have some alternatives", "Use $ares to use Ares, use the Ciphey CLI or use the Ares CLI.", false)
                    .footer(|f| f.text("http://discord.skerritt.blog"))
                    // Add a timestamp for the current time
                    // This also accepts a rfc3339 Timestamp
                    .timestamp(Timestamp::now())
